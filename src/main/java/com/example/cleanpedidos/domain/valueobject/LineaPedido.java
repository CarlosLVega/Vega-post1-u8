...
public record LineaPedido(String productoNombre, int cantidad, Dinero precioUnitario) {
    public LineaPedido {
        if (productoNombre == null || productoNombre.isBlank()) throw new IllegalArgumentException("El producto es obligatorio");
    ...
    public Dinero subtotal() { return new Dinero(precioUnitario.cantidad().multiply(BigDecimal.valueOf(cantidad))); }
}